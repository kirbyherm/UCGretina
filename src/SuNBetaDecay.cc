...
G4double SuNBetaDecay::GetMaximumElectronKineticEnergy(){
  ...
  G4double excitedStateEnergy = input_excitation_energies[cascade];
//  G4cout << excitedStateEnergy << G4endl;
...
G4double SuNBetaDecay::GetRandomElectronKineticEnergy(){

  const G4double maxKE = GetMaximumElectronKineticEnergy();
...
G4double SuNBetaDecay::GetPhaseSpaceElectronKineticEnergyDistribution(const G4double KE){
...
G4double SuNBetaDecay::GetPhaseSpaceElectronKineticEnergy(){
  ...
  for (unsigned int i=0; i<xValues.size(); i++){
    ...
    yValues[i] = GetPhaseSpaceElectronKineticEnergyDistribution(xValues[i]); 
  }
  ...
  while (true){
    ...
    randomNumC = GetPhaseSpaceElectronKineticEnergyDistribution(randomNumB); // Run randomNumB through the electron kinetic energy distribution 
...
G4double SuNBetaDecay::GetCorrectedElectronKineticEnergy(const G4String flag){
  ...
  for (unsigned int i=0; i<xValues.size(); i++){
    xValues[i] = (i/precision) * maxKE;
    if (flag == "A"){
      yValues[i] = GetApproximateFermiFunctionA(xValues[i]) * GetPhaseSpaceElectronKineticEnergyDistribution(xValues[i]); //  ==!!== Make sure that the Fermi function used here is the same as the one used directly above in the if statement and below in randomNumC ==!!==
      //G4cout << "using flag A the first time" << G4endl;
    }
    if (flag == "B"){
      yValues[i] = GetApproximateFermiFunctionB(xValues[i]) * GetPhaseSpaceElectronKineticEnergyDistribution(xValues[i]); //  ==!!== Make sure that the Fermi function used here is the same as the one used directly above in the if statement and below in randomNumC ==!!==
      //G4cout << "using flag B the first time" << G4endl;
    }
    if (flag == "C"){
      yValues[i] = GetApproximateFermiFunctionC(xValues[i]) * GetPhaseSpaceElectronKineticEnergyDistribution(xValues[i]); //  ==!!== Make sure that the Fermi function used here is the same as the one used directly above in the if statement and below in randomNumC ==!!==
